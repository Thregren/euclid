...
- **按 Apple HIG 过了一遍字号与对比度**（顺手把不合格的改掉）：图层行名称 12 → **13 点**
  （macOS 默认正文）、行内来源说明 10 → **11 点**，「基准」标记从 10 点三级色（实测对比度
  1.9:1，浅色下几乎看不见）改成 11 点带底胶囊（4.3:1 / 深色 5.1:1）；测量结果里的罗盘与转角
  从三级 / 四级色改为二级色（原来分别是 1.9:1 与 1.3:1）；检查器与三个面板里的说明文字
  10 → 11 点；勾选框、标题行的 ＋ / ⧉ / ⋯、工具条与画布控件都按 HIG「点击区域要够大」放大了一圈
- 删除右侧检查器里的旧「图层」面板与左侧边栏；可用数据 / 最近打开搬进检查器

### 1.13.0

- **界面按 Pixelmator Pro 的结构重做**：图层管理搬到右侧检查器顶部——
  「图层」面板的标题行右侧成组放「＋ 添加 / 复制 / ⋯ 更多」，下面是可拖拽排序、可点选的图层行
  （眼睛 + 类型图标 + 名称 + 来源 + 不透明度百分比），选中层的属性紧接在下方的「图层属性」里
  （名称 / 不透明度滑杆 / 来源 / 移除）。左侧侧栏只留「可用数据 / 位置 / 最近打开」
- 新增图层选中态、复制图层、拖拽排序、上移下移、全部显示 / 隐藏、移除选中层；
  工具栏的底图菜单改为「图层」菜单，第一段就是「添加图层」（在线预设 / 已打开的本地数据）
- 检查器分区顺序也按同一套逻辑重排：文档（图层）→ 选中对象属性 → 任务（测量）→ 读数（指针坐标 / 视图）→ 来源详情
