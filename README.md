...
当前版本 **1.14.5**，下载见 [Releases](https://github.com/Thregren/euclid/releases/latest)。
...
- **坐标基准**：底图若是高德 / 腾讯（GCJ-02）或百度（BD-09），在「下载在线瓦片」面板里选一下基准
  （在线浏览与下载共用同一套参数），就能与 WGS84 正射影像对齐，偏移方向与量级写在状态栏与检查器里
...
- **整块界面照 Pixelmator Pro 的分工**：画布铺满窗口，左侧「图层」、右侧检查器、最右缘竖直工具条
  都浮在画布之上，底部一条状态栏；工具条上五个工具一点即切（当前工具有底色），
  下面还有瓦片网格与「适配窗口」
...
./Scripts/run-checks.sh       # 运行自检（342 项；带一个数据集或影像路径时 350 项）
```

自检不需要 Xcode：`Sources/TileKitCheck` 是自带断言的可执行目标，覆盖投影往返、
相机变换、Vincenty 标准算例、TIFF 解码样本、Excel 包结构、下载计划与瓦片生成闭环。
...
左栏「图层」面板就是画布上所有东西的清单，顺序自上而下 = 画面从最上到最下：
...
| 显示 / 隐藏 | 行尾的勾选框 |
| 改不透明度 | 选中后用面板底部的滑杆 |
| 找某一层 | 底部搜索框按名字过滤，右边漏斗按种类过滤 |
| 添加 / 复制 / 移除 | 标题行的 ＋ / ⧉ / ⋯ |

测距 / 测面积 / 画圆画到一半时，**右键（或 ⌃+左键）即可结束**，不必移到别处双击；
测量列表里新画的排在最上面。

## 隐私与联网

- **不收集任何数据**：没有埋点、没有崩溃上报、没有检查更新，App 不会主动联网
- **网络只在你配置了在线底图 / 下载时才用**：请求只发往你选定的瓦片服务（天地图、OpenStreetMap
  或你自己填的 URL 模板），且只在需要取瓦片时发
- **数据都在本机**：打开的影像、测量结果、导出文件都留在本地；测量自动存档写在
  `~/Library/Application Support/Euclid/`，可随时删除
- **密钥只在内存里**：天地图的 tk 不落盘，退出程序即清空
- **本地瓦片服务只绑 127.0.0.1**：局域网里的其它机器连不上，且只读、只响应瓦片路径
- 仓库里不含任何真实测区数据：`瓦片/` 与构建产物都在 `.gitignore` 里，自检用的样本是
  合成 TIFF 与公开城市的通用坐标
...
| 检查器 | `InspectorView.swift`、`InspectorSections.swift`、`DataSourceSections.swift` | 测量、指针坐标、视图与来源详情、最近打开 |
...
`./Scripts/run-checks.sh` 覆盖 **342 项检查**（本机没有 XCTest，自检是一个自带断言的可执行目标）：
...
- **分段明细不再挤成一团**：窄面板里一行放不下「段 + 长度 + 方位 + 罗盘 + 转角」，
  现在拆成两行（主行给段、长度与方位，转角单独一行），并修掉一个数值 bug：
  度分秒没有进位，会出现 `106°60′`、`59.96″ → 60.0″` 这种写法。
  进位逻辑挪进核心库 `MeasureFormat`，自检新增 4 条断言覆盖它
- **拖动性能（彻底版）**：手势期间**完全不取图** —— 实测一次双指拖动里在途取图从几十片降到 **0**，
  只有手停下来那一次才去取。配合上一版的「只位移图层 + 读数攒着刷」，
  输入延迟稳定在 3.5–8 ms（原 25 ms）
- **右侧检查器瘦身**：删掉「图层属性」与「在线底图」两段 —— 不透明度在左栏面板底部的滑杆、
  改名与移除在图层行右键菜单、来源写在图层行副标题里；在线数据源 / 密钥 / **坐标基准**搬进
  「下载在线瓦片」面板（浏览与下载本来共用同一套参数）
- **左缘工具条悬停提示**：指针停在按钮上会显示「功能 + 快捷键 + 一句话说明」
  （例如「测距（D / ⌘3）：点击加点，右键、双击或回车结束」）。这条是自绘的，不依赖系统
  tooltip 的悬停时机；按钮上同时也挂了 `.help()`
- **两个图层菜单合并成一个**：工具栏上那个「图层」菜单并进左栏图层面板的 ＋ 里
  （添加图层 / 本地数据 / 打开 / 瓦片工具 / 显示瓦片网格）
- **代码梳理**：`AppModel` 从 1200 行拆成「状态中枢 + 图层 + 数据源 + 测量与出图」四个文件；
  三个面板各自的「分组框 + 字段标签」收敛成公共组件 `SheetChrome`；清掉全部编译警告与
  唯一的强制解包；补上「基准层有且只有一个」的不变量、手势被打断（切走应用 / 换窗口）时自动收尾、
  缩略图缓存上限与层级夹取
...
- **双指拖动（与左键拖动）平移跟手了**：实测原来每个滚轮事件都在做三件重活 —— 重铺瓦片、
  重绘测量标注、以及**逐个事件去改 SwiftUI 状态**（指针坐标 + 视图读数），
  结果事件被系统合并掉四分之三、输入延迟 25 ms（最慢 67 ms）。现在：
  - 手势期间只把已有图层整体位移（GPU 合成），位移攒到 120 点、手停下来或手势结束才提交给相机并重铺瓦片；
  - 坐标与视图读数改成「手停一下（120 ms）再刷」，手势期间一次都不碰 SwiftUI；
  - 手势期间不做瓦片预取，别和用户的手抢磁盘；
  - 顺带把检查器里读视图读数的那一段拆成独立子视图，避免整个 Form 跟着重算。
  同一套合成输入下：输入延迟 **25 ms → 3.5~5.4 ms**，80 个事件里被处理的数量 **18 → 54~69**。
- 新增 `EUCLID_TRACE_SCROLL=1` 打印每次平移手势的「事件数 / 处理耗时 / 输入延迟 / 提交次数」，
  `EUCLID_PAN_DIRECT=1` 可切回「每个事件都重铺」的老路径做对照，`EUCLID_PAN_NO_REPORT=1` 关掉读数刷新
...
- **右键结束测量**：画到一半（测距 / 测面积 / 画圆）直接**右键**就收尾，`⌃`+左键同样有效；
  双击与回车照旧。点数不够时按取消处理，和双击的口径一致
- **图层行右键菜单**：重命名、复制这一层、设为基准层、显示 / 隐藏、上移 / 下移、移除 ——
  对着哪一层想做什么，右键就到（原先只能先点中那一行，再翻面板顶上的 ⋯ 菜单）
- **测量列表倒序**：新画的排在最上面，画完抬头就能看到；存档与导出的顺序不变
- **修好「单独编辑样式」**：测量行的选中手势原先挂在整条上（含展开的明细），点「样式」三角、
  拖粗细滑杆、开颜色面板都会被它吃掉 —— 点一下就把这条测量取消选中、明细整块收起来，
  看上去就是「样式编辑坏了」。现在手势只挂在行标题上，顶点坐标 / 半径 / 分段明细 / 样式
  几个折叠块一起恢复正常
- 图层行同理：行尾的勾选框与不透明度不再被行的选中手势抢走
- 打包脚本：签名失败不再只打一行提示，而是直接报错退出（签名失败的 `.app` 一启动就被系统终止），
  并给出改用不受管理目录的绕开方式
